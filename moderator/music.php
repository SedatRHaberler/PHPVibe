...
<small>Filter</small>
    <ul class="dropdown-menu dropdown-right bullet" role="menu">
        ...
        <li role="presentation">
            <a title="Show premium only" href="<?php echo htmlspecialchars(add_sort('premium'), ENT_QUOTES, 'UTF-8'); ?>">
...
<?php
// Sanitize the input if it comes from a user-controlled source (e.g., $_GET or $_POST)
$ps = isset($_GET['ps']) ? $_GET['ps'] : '';  // Assuming 'ps' is a query parameter
$ps = htmlspecialchars($ps, ENT_QUOTES, 'UTF-8'); // Escape the value for safe HTML output

// Now pass it to the show_pages method
$a->show_pages($ps);
