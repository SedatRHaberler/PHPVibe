...
// Ensure 'checkRow' is set and is an array
    if (isset($_POST['checkRow']) && is_array($_POST['checkRow'])) {
        ...
        echo '<div class="msg-info">Performed ' . htmlspecialchars($act, ENT_QUOTES, 'UTF-8') . ' action on videos #' . htmlspecialchars(implode(',', $sanitized_checkRow), ENT_QUOTES, 'UTF-8') . '</div>';
    }}
...
echo '<div class="row-fuild" style="margin-bottom:15px"> Active filters:   ';
    foreach ($sortA as $filter) {
        ...
        // Sanitize the URL generated by remove_sort() if necessary
        $safe_url = htmlspecialchars(remove_sort($filter), ENT_QUOTES, 'UTF-8');

        // Safely output the sanitized $filter in the HTML
        echo '<a class="mright10" href="' . $safe_url . '">
