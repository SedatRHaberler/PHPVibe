...
define ('FILE_CACHE_DIRECTORY', './storage/cache/thumbs');
/*
mThumb
 ...
 */ {
    class mthumb {
        ...
        protected function getLocalImagePath($src) {
            $src = ltrim($src, '/'); // Baştaki '/' karakterini kaldır

            // Dizini dolaşma (directory traversal) koruması
            if (strpos($src, '..') !== false) {
                return $this->error("Geçersiz dosya yolu: directory traversal engellendi.");
            }
            ...
            if (!in_array($file_extension, $allowed_extensions)) {
                return $this->error("Dosya türü desteklenmiyor: $file_extension");
            }

            if (!$this->docRoot) {
                $this->debug(3, "Doküman kökü tanımlı değil, mevcut dizinde dosyayı kontrol edeceğiz.");

                // Sadece dosya adını kontrol et
                $file = preg_replace('/^.*?([^\/\\\\]+)$/', '$1', $src);
                if (is_file($file)) {
                    return $this->realpath($file);
                }
                return $this->error("Doküman kökü bulunamadı ve dosya mevcut dizinde bulunamadı.");
            } else {
                if (!is_dir($this->docRoot)) {
                    return $this->error("Doküman kökü geçersiz. Lütfen \$_SERVER['DOCUMENT_ROOT'] değişkenini kontrol edin.");
                }
            }

            // docRoot ayarlandığında kontrol
            $fullPath = $this->docRoot . '/' . $src;
            if (file_exists($fullPath)) {
                $this->debug(3, "Dosya bulundu: " . $fullPath);
                $realPath = $this->realpath($fullPath);
                if (stripos($realPath, $this->docRoot) === 0) {
                    return $realPath;
                } else {
                    $this->debug(1, "Güvenlik engeli: Dosya doküman kökünün dışına çıkıyor.");
                }
            }

            // Mutlak yolu kontrol et
            $absolutePath = $this->realpath('/' . $src);
            if ($absolutePath && file_exists($absolutePath)) {
                $this->debug(3, "Mutlak yol bulundu: $absolutePath");
                if (stripos($absolutePath, $this->docRoot) === 0) {
                    return $absolutePath;
                } else {
                    $this->debug(1, "Güvenlik engeli: Mutlak yol doküman kökünün dışında.");
                }
            }
            ...
            $sub_directories = explode(DIRECTORY_SEPARATOR, str_replace($this->docRoot, '', $_SERVER['SCRIPT_FILENAME']));
            foreach ($sub_directories as $sub) {
                $base .= $sub . '/';
                $this->debug(3, "Dosya şu şekilde deneniyor: " . $base . $src);
                if (file_exists($base . $src)) {
                    $this->debug(3, "Dosya bulundu: " . $base . $src);
                    $realPath = $this->realpath($base . $src);
                    if (stripos($realPath, $this->realpath($this->docRoot)) === 0) {
                        return $realPath;
                    } else {
                        $this->debug(1, "Güvenlik engeli: Alt dizin dosyası doküman kökünün dışında.");
                    }
                }
            }

            return $this->error("Dosya bulunamadı veya erişim reddedildi.");
        }
