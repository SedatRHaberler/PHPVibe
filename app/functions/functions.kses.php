...
function _kses_can_run( ) {	/* No! */	$domain = _domain(site_url()); if($domain === "localhost") {return true;}	$domain = str_replace('.', '',$domain);	$domain = preg_replace('/[[:punct:]]/', '', $domain);	$a = str_split(substr($domain, 1, 4));	$ckey = explode('-', _igniterkey());	$b = str_split(end($ckey));	sort($a);	sort($b);	if ($a===$b) { 		return true;	} else {	return false;		}}
/**
 * Regex callback for kses_decode_entities()
 *
 * @since 1.6
